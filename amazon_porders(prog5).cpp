...
Due Date:  2/23/22                    Due Time:11:59PM
Total Points: 10
Assignment #: 5 amazon_porders.cpp
...
The function will then process that information and produce an output file of the orders in the source file.
...
void output(ofstream& out, const order_record& p_record)
{
	...
	out << p_record.cell_number << "\t";
	out << p_record.item_number << "\t";
	out << p_record.quantity << "\t";
	out << p_record.price << "\t";
	out << p_record.processing_plant << "\t";
	out << p_record.tax_rate * 100 << "\t";
	out << p_record.order_tax << "\t";
	out << p_record.net_cost << "\t";
	out << p_record.total_cost << "\t";
...
int main()
{
	...
	ofstream out("amazoninventory_output.txt");
